...
func TestDecodingStreamer_ParseConnectPacket(t *testing.T) {
	reader := bytes.NewReader([]byte{
		// connect packet
		16,   // connect + 0 flags
		29,   // remaining length
	...
	streamer := NewDecodingStreamer(reader)

	header, err := streamer.Next()
	if err != nil {
		t.Fatal("unexpected error", err)
	}
	if header == nil {
		t.Fatal("header is nil", err)
	}

	p, err := streamer.DecodePacket()
	if err != nil {
		t.Fatal("unexpected error", err)
	}
	if p == nil {
		t.Fatal("expected packet to be not nil")
	}
